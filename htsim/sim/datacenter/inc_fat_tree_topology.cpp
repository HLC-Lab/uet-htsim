...
std::ostream &operator<<(std::ostream &os, IncFatTreeTopologyCfg const &m) { 
    os << "IncFatTreeTopologyCfg"
       << " NCORE=" << m.NCORE 
        << " NAGG=" << m.NAGG
        << " NTOR=" << m.NTOR
        << " NSRV=" << m.NSRV
...
IncFatTreeTopologyCfg::IncFatTreeTopologyCfg(queue_type q, queue_type snd):
                        _from_file(false),
                        _qt(q),
                        _sender_qt(snd),
                        NCORE(0), 
...
IncFatTreeTopologyCfg::IncFatTreeTopologyCfg(uint32_t tiers, uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize,
                                       simtime_picosec latency, simtime_picosec switch_latency, 
                                       queue_type q, queue_type snd):
                                       IncFatTreeTopologyCfg(q, snd) {
    initialize(tiers, no_of_nodes, linkspeed, queuesize, latency, switch_latency, q, snd);
}

IncFatTreeTopologyCfg::IncFatTreeTopologyCfg(istream& file, mem_b queue_size,
                                       queue_type q, queue_type snd):
                                       IncFatTreeTopologyCfg(q, snd) {
...
void IncFatTreeTopologyCfg::initialize(uint32_t tiers, uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize,
                                    simtime_picosec latency, simtime_picosec switch_latency, 
                                    queue_type q, queue_type snd) {
    set_tiers(tiers);
...
IncFatTreeTopologyCfg::IncFatTreeTopologyCfg(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize,
                                       queue_type q):
                                       IncFatTreeTopologyCfg(q, FAIR_PRIO) {
    set_linkspeeds(linkspeed);
    set_queue_sizes(queuesize);
    _num_failed_links = 0;
    if (_link_latencies[TOR_TIER] == 0) {
...
void IncFatTreeTopologyCfg::set_custom_params(uint32_t no_of_nodes) {
...
void
IncFatTreeTopologyCfg::set_tier_parameters(int tier, int radix_up, int radix_down, mem_b queue_up, mem_b queue_down, int bundlesize, linkspeed_bps linkspeed, int oversub) {
...
void IncFatTreeTopologyCfg::set_linkspeeds(linkspeed_bps linkspeed) {
...
void IncFatTreeTopologyCfg::set_queue_sizes(mem_b queuesize) {
...
void IncFatTreeTopologyCfg::set_params(uint32_t no_of_nodes) {
    if (_hosts_per_pod > 0) {
...
simtime_picosec IncFatTreeTopologyCfg::get_two_point_diameter_latency(int src, int dst) {
...
unique_ptr<IncFatTreeTopologyCfg> IncFatTreeTopologyCfg::load(string filename,
                                                        mem_b queuesize,
                                                        queue_type q_type,
                                                        queue_type sender_q_type) {
    std::ifstream file(filename);
    if (file.is_open()) {
        unique_ptr<IncFatTreeTopologyCfg> cfg = make_unique<IncFatTreeTopologyCfg>(file, queuesize, q_type, sender_q_type);
...
void IncFatTreeTopologyCfg::read_cfg(istream& file, mem_b queuesize) {
...
void IncFatTreeTopologyCfg::check_consistency() const {
...
IncFatTreeTopology::IncFatTreeTopology(const IncFatTreeTopologyCfg* cfg,
...
IncFatTreeTopology::~IncFatTreeTopology() {
...
void IncFatTreeTopology::alloc_vectors() {
...
BaseQueue* IncFatTreeTopology::alloc_src_queue(QueueLogger* queueLogger){
...
BaseQueue* IncFatTreeTopology::alloc_queue(QueueLogger* queueLogger, const mem_b queuesize,
...
BaseQueue*
IncFatTreeTopology::alloc_queue(QueueLogger* queueLogger, linkspeed_bps speed, const mem_b queuesize_param,
...
vector<const Route*>* IncFatTreeTopology::get_bidir_paths(uint32_t src, uint32_t dest, bool reverse){
...
void IncFatTreeTopology::count_queue(Queue* queue){
...
int64_t IncFatTreeTopology::find_lp_switch(Queue* queue){
    //first check ns_nlp
...
int64_t IncFatTreeTopology::find_up_switch(Queue* queue){
    count_queue(queue);
    //first check nc_nup
...
int64_t IncFatTreeTopology::find_core_switch(Queue* queue){
    count_queue(queue);
    //first check nup_nc
    for (uint32_t agg=0;agg<_cfg->NAGG;agg++)
        for (uint32_t core = 0;core<_cfg->NCORE;core++)
...
int64_t IncFatTreeTopology::find_destination(Queue* queue){
    //first check nlp_ns
    for (uint32_t tor=0; tor<_cfg->NTOR; tor++)
        for (uint32_t srv = 0; srv<_cfg->NSRV; srv++)
...
void IncFatTreeTopology::print_path(std::ofstream &paths,uint32_t src,const Route* route){
    paths << "SRC_" << src << " ";
...
void IncFatTreeTopology::add_switch_loggers(Logfile& log, simtime_picosec sample_period) {
